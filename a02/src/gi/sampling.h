...
inline glm::vec3 uniform_sample_hemisphere(const glm::vec2& sample) {
	...
	// NOTE: biased results as more samples are taken towards the poles
...
inline glm::vec3 cosine_sample_hemisphere(const glm::vec2& sample) {
	...
	// Malley's method: sample a point on the unit disk and lift it to the hemisphere -> biased with cosine -> more samples around normal
    glm::vec2 disk = concentric_sample_disk(sample);
    //glm::vec2 disk = uniform_sample_disk(sample);

    // Project the disk point up to the hemisphere
    float z = sqrtf(fmaxf(0.0f, 1.0f - disk.x * disk.x - disk.y * disk.y));
